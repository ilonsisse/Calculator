...
class Calculator(QWidget):
    def __init__(self):
        ...
        self.b_divine = QPushButton("/", self)
        self.hbox_second.addWidget(self.b_divine)
        ...
        self.b_divine.clicked.connect(lambda: self._operation("/"))
    ...
    def _operation(self, op):
        self.op = op
        if self.op != 'C':
            if '.' in self.input.text() and all(list(map(lambda x: x.isdigit(), self.input.text().split('.')))):
                self.num_1 = float(self.input.text())
            elif self.input.text().isdigit():
                self.num_1 = int(self.input.text())
            else:
                self.input.setText("Введите число")
        self.input.setText("")
